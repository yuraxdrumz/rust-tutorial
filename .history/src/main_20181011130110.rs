...
fn main() {
    ...
    let id = bson::oid::ObjectId::with_string("5b9117cca9f24f3771577bb0").unwrap();

    println!("id: {}", id);

    let user = users_coll.find(Some(doc! { "_id" => id}), None).unwrap();

    for (i, result) in user.enumerate() {
